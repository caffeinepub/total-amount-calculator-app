...
export default function LoginButton() {
      ...
      try {
        ...
        if (error.message === "User is already authenticated") {
          try {
            ...
            setUserVisibleError(
              "Authentication state error. Please refresh the page.",
            );
          }
        } else if (error.message?.includes("AuthClient is not initialized")) {
          setUserVisibleError(
            "Login system is still initializing. Please wait a moment and try again.",
          );
  ...
  const displayError =
    userVisibleError || (loginError ? loginError.message : null);
